...
import { eurovision2021Data } from '@/data/eurovision2021';
...
export default function Eurovision2021() {
  ...
  const pendingKey = 'eurovision2021_pending_votes';
  ...
  const openYouTubeModal = (country: string) => {
    const songData = eurovision2021Songs[country];
  ...
  useEffect(() => {
    async function fetchConfig() {
      try {
        console.log(`[Eurovision2021] Fetching config for year: 2021`);
        console.log(`[Eurovision2021] User authentication status: ${status}`);
        console.log(`[Eurovision2021] User email: ${session?.user?.email || 'Not signed in'}`);
        
        // Add a timestamp to prevent caching issues
        const response = await fetch(`/api/config/vote-config?year=2021&t=${Date.now()}`);
        if (response.ok) {
          const data = await response.json();
          console.log(`[Eurovision2021] Config API response:`, data);
          console.log(`[Eurovision2021] isGM status: ${data.isGM}`);
          console.log(`[Eurovision2021] Mode setting: ${data.mode}`);
          setVoteConfig(data);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    async function tryResendPending() {
      try {
        ...
        const resp = await fetch('/api/votes/2021', {
          method: 'POST',
  ...
  const updateResults = async () => {
    ...
    const allCountries = Object.keys(eurovision2021Songs);
    ...
    try {
      ...
      const response = await fetch('/api/votes/2021', {
        method: 'POST',
        headers: {
  ...
  const fetchFreshResults = async () => {
    try {
      ...
      const endpoint = '/api/votes/2021/simple';
  ...
  const fetchResults = async (retryCount = 0) => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.totalVotes === 0) {
          ...
          if (timeSinceLastRetry >= 60000) {
            ...
            try {
              ...
              if (debugData.focus2021?.votesCount > 0) {
        ...
        const savedShowResults = localStorage.getItem('eurovision2021_showResults');
        if (savedShowResults !== null) {
          ...
          console.log(`[Eurovision2021] Saved preference is to ${shouldShowResults ? 'show' : 'hide'} results`);
          console.log(`[Eurovision2021] Current mode: ${voteConfig.mode}, isGM: ${voteConfig.isGM}`);
          
          if (shouldShowResults && voteConfig.mode === 'hide' && !voteConfig.isGM) {
            // Don't show results if mode is hide (unless user is GM)
            setShowResults(false);
            console.log('[Eurovision2021] Results hidden due to configuration mode: hide');
          } else {
            setShowResults(shouldShowResults);
            console.log(`[Eurovision2021] Setting showResults to: ${shouldShowResults}`);
          }
        }
  ...
  const toggleShowResults = () => {
    ...
    localStorage.setItem('eurovision2021_showResults', JSON.stringify(newShowResults));
  ...
  // Get all countries from the eurovision2021Songs mapping
  const allCountries = Object.keys(eurovision2021Songs);
          ...
          <DragDropContext onDragEnd={handleDragEnd}>
            <div className="flex flex-wrap gap-8">
              ...
              <div className="w-full lg:w-[420px]">
                <NextVotingSelector currentYear="2021" />
                <div className="bg-[#2c3e50] rounded-lg p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <h2 className="text-2xl font-bold text-white mb-0">Oylarım</h2>
                                ...
                                {selectedCountries[index] ? (
                                  ...
                                  >
                                    {(provided, snapshot) => (
                                      ...
                                      >
                                        <Image 
                                          src={`/flags/${selectedCountries[index].replace('&', 'and')}_${eurovision2021Songs[selectedCountries[index]]?.code}.png`}
                                  ...
                                  >
                                    <div className="flex items-center gap-2">
                                      ...
                                      <div className="flex-shrink-0 flex flex-col items-center">
                                        <Image 
                                          src={`/flags/${country.replace('&', 'and')}_${eurovision2021Songs[country]?.code}.png`}
                                          alt={`${country} flag`}
                                          width={24}
                                          height={16}
                                        ...
                                        />
                                        {eurovision2021Songs[country]?.youtubeId && (
                                          <button 
                                      ...
                                      <div className="flex flex-col min-w-0 flex-1">
                                        ...
                                        {eurovision2021Songs[country] && (
                                          <div className="flex flex-col">
                                            <span className="text-xs text-gray-400 truncate">
                                              {eurovision2021Songs[country].performer}
                                            </span>
                                            <span className="text-xs text-gray-500 truncate">
                                              {eurovision2021Songs[country].song}
                                            </span>
                                          </div>
                                        )}
                                      </div>
      ...
      {showYouTubeModal && (
        ...
        >
          ...
          >
            ...
            <h3 className="text-xl font-bold mb-4 text-white">{selectedCountryName} - Eurovision 2021</h3>
            <div className="aspect-video">
              <iframe
                width="100%"
                height="100%"
                src={`https://www.youtube.com/embed/${selectedVideoId}?hd=1&quality=hd720`}
                title={`${selectedCountryName} Eurovision 2021 Performance`}
                frameBorder="0"
