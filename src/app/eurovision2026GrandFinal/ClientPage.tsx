...
export default function Eurovision2026GrandFinal() {
  ...
  const toggleRevealResult = (country: string) => {
    ...
    if (willReveal) lastRevealedRef.current = country;
          ...
          {sortedCountries.map(([country, points], index) => {
            ...
            return (
              <div
                ...
                className={`flex items-center gap-3 rounded-xl border ${medal.border} bg-white/4 backdrop-blur-sm shadow-lg ${medal.glow ? `shadow-lg ${medal.glow}` : ''} transition-all duration-300 overflow-hidden cursor-pointer hover:bg-white/8 active:scale-[0.99] ${isTop3 ? 'p-3.5' : 'p-2.5'}`}
                style={isTop3 ? { boxShadow: `0 0 20px ${rank === 1 ? 'rgba(255,215,0,0.12)' : rank === 2 ? 'rgba(200,200,200,0.08)' : 'rgba(205,127,50,0.1)'}` } : undefined}
              >
                {/* Rank badge */}
                <div
                ...
                <div className="flex-shrink-0 text-right min-w-[80px]">
                  {revealed ? (
                    <div className="space-y-0.5">
                      ...
                      <div className="text-xs text-gray-500 leading-tight">puan</div>
                      {(() => {
