...
export async function GET() {
  try {
    ...
    const gmList = VOTE_CONFIG?.['2026']?.GMs
      ? VOTE_CONFIG['2026'].GMs.split(',').map((e) => e.trim().toLowerCase()).filter(Boolean)
      : [];
    ...
    const cumulativeResults = await dbStorage.getCumulativeResults(2026);
    ...
    console.error('Error in GET /api/votes/2026/public:', error);
