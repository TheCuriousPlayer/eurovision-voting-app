...
export async function GET() {
  try {
    ...
    // Get the 2020 Semi-Final B competition by year
    const competition = await prisma.competition.findFirst({
      where: { year: 202002 }
    });

    if (!competition) {
      console.error('202002 competition not found in database');
      return NextResponse.json({ 
        countryPoints: {}, 
        totalVotes: 0,
        userVote: null,
    ...
    console.log('Database query results for 202002:');
    ...
    const voteConfig = VOTE_CONFIG['202002'];
    ...
    console.error('Error in simple API for 202002:', error);
