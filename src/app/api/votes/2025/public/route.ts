...
export async function GET() {
  try {
    ...
    const gmList = VOTE_CONFIG?.['2025']?.GMs
      ? VOTE_CONFIG['2025'].GMs.split(',').map((e) => e.trim().toLowerCase()).filter(Boolean)
      : [];
    ...
    const cumulativeResults = await dbStorage.getCumulativeResults(2025);
    ...
    console.error('Error in GET /api/votes/2025/public:', error);
