...
import PageReadySignal from '@/components/PageReadySignal';
  ...
  const eurovision2020Songs = eurovision2020DataGroupA;
...
export default function Eurovision2020SemiFinalA() {
  ...
  const pendingKey = 'eurovision2020_semi_final_a_pending_votes';
  ...
  useEffect(() => {
    async function fetchConfig() {
      try {
        ...
        const response = await fetch(`/api/config/vote-config?year=202001&t=${Date.now()}`);
        if (response.ok) {
  ...
  useEffect(() => {
    ...
    async function tryResendPending() {
      ...
      try {
        ...
        const resp = await fetch('/api/votes/202001', {
          method: 'POST',
  ...
  const updateResults = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/votes/202001', {
        method: 'POST',
        headers: {
  ...
  const fetchFreshResults = async () => {
    try {
      ...
      const endpoint = '/api/votes/202001/simple';
  ...
  const handleDragEnd = (result: DropResult) => {
    // If voting is closed, ignore any drag attempts
    if (votingClosed) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1a1a2e] to-[#16213e] py-8">
      ...
      <div className="container mx-auto px-4">
        ...
        <EurovisionNavigation currentPage="2020-semi-final-a" />
